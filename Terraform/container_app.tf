...
# terraform import azurerm_container_app.app "/subscriptions/27de852c-c023-4de6-ac4d-52c9d4470482/resourceGroups/teeledger-q-rg/providers/Microsoft.App/containerApps/teeledger-q-ca-server"
resource "azurerm_container_app" "app" {
  name                            = "${local.name_prefix}-ca-app"
  resource_group_name             = azurerm_resource_group.main.name
  ...
  template {
    container {
      name   = "${local.name_prefix}-c-app"
      image  = "${var.ghcr_url}/${var.ghcr_username}/${var.image}"
      cpu    = "0.5"
      memory = "1Gi"
      env {
