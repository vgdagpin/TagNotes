...
const TnSection = ({ section, isNew, onSaveSection, onDeleteSection }: TnSectionProps) => {
  ...
  return (
    <div
      ref={rootRef}
      className="note-section rounded-md p-2 transition-colors min-w-0 w-full bg-transparent focus:outline-none focus:ring-1 focus:ring-accent"
    ...
    >
      {sectionEdit ? (
        ...
        <div className="whitespace-pre-wrap text-sm leading-relaxed" style={{ background: 'white' }}>
          {section.content || <span className="text-muted-foreground italic">Blank..</span>}
        </div>
      )}
    </div>
  );
