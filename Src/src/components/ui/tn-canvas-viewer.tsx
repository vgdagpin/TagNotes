...
const TnCanvasViewer: React.FC<CanvasViewerProps> = ({
	...
	const [provisionalBottom, setProvisionalBottom] = useState(0);
	const [provisionalRight, setProvisionalRight] = useState(0);
	const spacerHeight = Math.max(maxBottom, provisionalBottom) + 200; // modest buffer
	const contentWidth = Math.max(maxRight, provisionalRight) + 400; // modest horizontal buffer
		...
		(section: Section) => {
						...
						return (
							<TnSectionCode
								...
								onSaveSection={(content, language) =>
									onSaveSection(section.id, content, language)
								}
			...
			return (
				<TnDraggableSection
					...
					onProvisionalExtentChange={(r, b) => {
						if (r > 0) setProvisionalRight(r);
						if (b > 0) setProvisionalBottom(b);
					}}
				>
					{sectionComponent}
				</TnDraggableSection>
	...
	return (
		<div className="canvas-outer relative w-full h-full">
			{/* removed overflow-hidden so inner scrollbars show */}
			{/* Canvas */}
			<div
				ref={canvasRef}
				className="canvas-inner-container relative w-full bg-gradient-to-br from-gray-50 to-gray-100 overflow-auto cursor-crosshair"
				style={{
					backgroundImage: `
          ...
          `,
					...
					// ensure scroll container itself can expand horizontally if needed
					minWidth: '100%',
					height: '100%',
				}}
				...
				{note.sections.length === 0 && (
					<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
						<div className="text-center text-gray-500">
							...
							<p className="text-sm">
								Create text, markdown, or code sections by clicking on the canvas
							</p>
						</div>
					</div>
				)}
