...
const TnNavigation = ({
	...
	const [isDirectoryLoaded, setIsDirectoryLoaded] = useState<boolean | undefined>(
		directoryLoaded,
	);
		...
		const run = async () => {
			...
			try {
				if (isDirectoryLoaded) {
					...
					list = [...list].sort(
						(a, b) => (b.createdAt as any).getTime() - (a.createdAt as any).getTime(),
					);
				...
				<NavDrawerHeader>
					<div className="flex items-center justify-between mb-4">
						<h1 className="text-xl font-semibold text-foreground flex items-center gap-2">
							...
							TagNotes{' '}
							{isDirectoryLoaded && (
								<span className="text-xs text-green-600 border rounded px-1">
									Local
								</span>
							)}
						</h1>
