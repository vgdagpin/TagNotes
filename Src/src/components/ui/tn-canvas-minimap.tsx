...
const TnCanvasMinimap: React.FC<CanvasMinimapProps> = ({ sections, canvasRef, width = 200, height = 140 }) => {
  const [viewport, setViewport] = useState({ x: 0, y: 0, w: 0, h: 0, scale: 1 });
  ...
  useEffect(() => {
    ...
    for (const s of sections) {
      const sx = s.x ?? 0; const sy = s.y ?? 0; const sw = s.width ?? 400; const sh = s.height ?? 200;
      if (sx < minX) minX = sx;
      if (sy < minY) minY = sy;
    ...
    setBounds({ minX: Math.max(0, minX - pad), minY: Math.max(0, minY - pad), maxX: maxX + pad, maxY: maxY + pad });
  }, [sections]);
  ...
  const centerOn = (relX: number, relY: number, smooth=true) => {
    ...
    el.scrollTo({ left: Math.max(0, targetCanvasX), top: Math.max(0, targetCanvasY), behavior: smooth ? 'smooth' : 'auto' });
  };
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    const rect = (e.currentTarget as HTMLDivElement).getBoundingClientRect();
    dragState.current = { active: true, startX: e.clientX - rect.left, startY: e.clientY - rect.top };
  ...
  const handleMouseUp = () => { if (dragState.current) dragState.current.active = false; };

  return (
    <div className={`rounded-md border border-gray-300 bg-white/70 backdrop-blur-sm shadow-sm overflow-hidden group/minimap transition-all duration-200 ${collapsed ? 'opacity-60 hover:opacity-100 cursor-pointer' : ''}`}
      style={{ width: collapsed ? 90 : width, height: collapsed ? 32 : height }}
      title="Minimap"
    >
      <div className="relative w-full h-full" style={{ background: collapsed ? 'transparent' : 'repeating-linear-gradient(45deg,#f8fafc,#f8fafc 6px,#f1f5f9 6px,#f1f5f9 12px)' }}
      ...
      >
          ...
          return (
            <div
              ...
              style={{
                ...
                width: Math.max(2, w),
                height: Math.max(2, h),
                background: s.type === 'image' ? 'rgba(16,185,129,0.45)' : s.type === 'code' ? 'rgba(59,130,246,0.45)' : s.type === 'markdown' ? 'rgba(139,92,246,0.45)' : 'rgba(107,114,128,0.45)',
        ...
        />
        {/* Scale indicator */}
        <div className="absolute bottom-0 right-0 text-[10px] px-1 py-0.5 bg-white/70 text-gray-600 rounded-tl">{(1/viewport.scale || 0).toFixed(1)}x</div>
        </>)}
      </div>
    </div>
  );
