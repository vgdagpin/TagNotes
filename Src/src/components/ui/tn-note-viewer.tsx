...
const TnNoteViewer = ({ note, directoryLoaded, onTitleUpdated }: TnNoteViewerProps) => {
	...
	const addImageSection = async (noteId: string, imageData: string) => {
		...
		const newImageSection = await tagNotesContext.addImageSection(
			noteId,
			imageData,
			DEFAULT_IMAGE_WIDTH,
	...
	const handleAddSection = async (x: number, y: number, sectionType: Section['type']) => {
		...
		const newSection = await tagNotesContext.addSection(
			selectedNote.id,
			sectionType,
			DEFAULT_WIDTH,
			DEFAULT_HEIGHT,
	...
	const handlePositionChange = async (sectionId: string, x: number, y: number) => {
		...
		setSelectedNote((prev) => ({
			...prev,
			sections: prev.sections.map((sec) => (sec.id === sectionId ? { ...sec, x, y } : sec)),
		}));
	};
	...
	const handleSaveSection = async (
		...
		setSelectedNote((prev) => ({
			...prev,
			sections: prev.sections.map((sec) =>
				sec.id === sectionId ? { ...sec, content, language } : sec,
			),
		}));
	...
	return (
		...
		>
			<div className="note-viewer-container">
				...
				<div className="canvas-container flex-1 min-w-0 w-full relative">
					<TnCanvasViewer
						note={selectedNote}
