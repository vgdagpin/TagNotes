...
function registerIpcHandlers() {
	...
	// ---- Helper functions ----
	function indexPath(dirPath) {
		return path.join(dirPath, 'index.json');
	}
	function notesDir(dirPath) {
		return path.join(dirPath, 'Notes');
	}
	...
	function loadIndex(dirPath) {
		ensureIndex(dirPath);
		try {
			return JSON.parse(fs.readFileSync(indexPath(dirPath), 'utf-8'));
		} catch {
			return { notes: [] };
		}
	}
	function saveIndex(dirPath, data) {
		fs.writeFileSync(indexPath(dirPath), JSON.stringify(data, null, 2), 'utf-8');
	}
	function loadNote(location) {
		return JSON.parse(fs.readFileSync(location, 'utf-8'));
	}
	function saveNote(location, note) {
		fs.writeFileSync(location, JSON.stringify(note, null, 2), 'utf-8');
	}
	function touchUpdated(indexData, id, updatedAt) {
		const entry = indexData.notes.find((n) => n.id === id);
		if (entry) {
	...
	ipcMain.handle(
		'add-image-section',
		(event, dirPath, noteId, imageData, width, height, x, y) => {
			const idx = loadIndex(dirPath);
			const entry = idx.notes.find((n) => n.id === noteId);
			if (!entry) return null;
			const note = loadNote(entry.location);
	...
	ipcMain.handle(
		...
		(event, dirPath, noteId, sectionId, content, language) => {
			const idx = loadIndex(dirPath);
			const entry = idx.notes.find((n) => n.id === noteId);
	...
	ipcMain.handle('delete-section', (event, dirPath, noteId, sectionId) => {
		...
		if (note.sections.length === before) {
			return false;
		}
		note.updatedAt = new Date();
		touchUpdated(idx, noteId, note.updatedAt);
	...
	ipcMain.handle('delete-note', (event, dirPath, noteId) => {
		const idx = loadIndex(dirPath);
		const i = idx.notes.findIndex((n) => n.id === noteId);
		if (i === -1) return false;
		const entry = idx.notes[i];
		try {
			if (entry.location && fs.existsSync(entry.location)) fs.unlinkSync(entry.location);
		} catch {}
